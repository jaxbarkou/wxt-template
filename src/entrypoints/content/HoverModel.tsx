...
const HoverModel: React.FC<AppProps> = ({ symbol, onClose }) => {
  ...
  const { hoverModelDisabled } = useWxtStorage();
    ...
    return (
      <div className="relative flex items-center gap-3">
        ...
        <div className="relative w-5 h-5">
          ...
          {/* 下拉展示设置卡片 */}
          {showHoverCard && (
            <div className="absolute left-[-180px] top-[28px] z-[100000] w-[260px] h-[80px]">
              <DisplayHoverCard
                changeConfig={onClose}
                className="w-[260px] h-[75px] rounded-[8px] shadow-[0px_0px_8px_#00000029]"
              />
            </div>
          )}
        </div>
                        ...
                        <div className="flex items-center gap-3">
                          <Avatar className="w-10 h-10 bg-brand-primary">
                            ...
                            <AvatarFallback className="text-white bg-brand-primary">
                              {projectData?.project_info?.name?.charAt(0) ||
                                "Y"}
                            </AvatarFallback>
                          </Avatar>
                          <div>
