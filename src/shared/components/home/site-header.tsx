...
import { LanguageSwitcher } from "@/components/alia/language-switcher";
    ...
    <header className="supports-backdrop-blur:bg-background/80 bg-background/40 sticky top-0 left-0 z-40 flex h-15 w-full flex-col items-center backdrop-blur-lg">
      <div className="container flex h-15 items-center justify-between px-3">
        ...
        <div className="relative flex items-center gap-2">
          ...
          >
            <a href="https://github.com/bytedance/alia" target="_blank">
              <GitHubLogoIcon className="size-4" />
              {t('starOnGitHub')}
              {env.NEXT_PUBLIC_STATIC_WEBSITE_ONLY &&
                env.GITHUB_OAUTH_TOKEN && <StarCounter />}
            </a>
            {/* <Link href="https://github.com/bytedance/alia" target="_blank">
              <GitHubLogoIcon className="size-4" />
              {t('starOnGitHub')}
              {env.NEXT_PUBLIC_STATIC_WEBSITE_ONLY &&
                env.GITHUB_OAUTH_TOKEN && <StarCounter />}
...
export async function StarCounter() {
  ...
  try {
    const response = await fetch(
      "https://api.github.com/repos/bytedance/alia",
      {
