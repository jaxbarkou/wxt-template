...
export const fetchBloomFilterWithCache = async (): Promise<
  Record<ProjectsQueryType, string>
> => {
  ...
  try {
    ...
    if (result[CACHE_KEY] && result[TIMESTAMP_KEY]) {
      ...
      if (age < CACHE_DURATION) {
        ...
        console.log(
          "缓存数据大小:",
          JSON.stringify(result[CACHE_KEY]).length,
          "bytes"
        );
...
// 获取新鲜数据
const fetchFreshBloomFilterData = async (): Promise<
  Record<ProjectsQueryType, string>
> => {
  const queryList = [
